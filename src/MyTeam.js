...
function MyTeam({ userinfo }) {
  ...
  useEffect(() => {
    axios
      .get("http://localhost:8000/user/team", {
  ...
  const handleDelete = (index) => {
    ...
    axios
      .patch(
        "http://localhost:8000/user/team",
        { team: newTeam, email: userinfo.email },
        { withCredentials: true }
      )
      .then((resp) => {
    ...
    <div className="h-full w-full flex flex-col items-center ">
      <div className="m-2 h-auto flex justify-center">
        {selectedTeam === undefined ? (
          <h3>Please build and select a team</h3>
        ) : (
      ...
      <div className="w-full h-full flex">
        <div className="w-[60%]">
          {teams &&
            ...
            teams.map((team, index) => {
              return (
                <div className="h-auto m-4 flex w-full">
                  {teams.length > 0 && (
                    <PokemonSlots
                      team={teams[index]}
        ...
        <div className="w-[40%] h-[75%] bg-green-400 mr-6"></div>
      </div>
    </div>
  );
}
