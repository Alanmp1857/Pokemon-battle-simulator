...
function Battle() {
  ...
  useEffect(() => {
    ...
    let arr = [];
    Object.keys(team1).map((id) =>
      team1[id].moves.map((move) => move != "" && arr.push(move))
    );
    // console.log(arr)
    axios
      .all(arr.map((move) => axios.get(`${baseUrl}/moves/${move}`)))
      .then((resp) => {
        resp.map((res) => {
          let obj = {};
          obj["PP"] = res.data.PP;
  ...
  useEffect(() => {
    if (ind >= 0) {
      ...
      socket.emit("pokemon", {
        ...
        roomid: roomid,
        currHp: (pokemonhp[ind][1] / pokemonhp[ind][2]) * 100,
      });
  ...
  useEffect(() => {
    if (oppAttkData != undefined) {
      ...
      let power =
        data.moveinfo.Power == "—" ? 0 : parseInt(data.moveinfo.Power);
      const attack =
        data.moveinfo.Category === "physical" ? data.attk : data.spa;
      const defence =
        data.moveinfo.Category === "physical"
          ? pokemoninformation.stats[2]
          : pokemoninformation.stats[4];
      let damage =
        (0.25 * power * (attack / defence) * crit * stab * thp * probability) /
        100;
  ...
  const handleAttack = (moveinfo) => {
    socket.emit("attack", {
      moveinfo: moveinfo,
      attk: pokemoninfo.stats[1],
      spa: pokemoninfo.stats[3],
      pokemontype: pokemoninfo.type,
      roomid: roomid,
    });
  };
    ...
    <div className="h-full w-full bg-slate-800 flex">
      <div className="h-full min-w-auto flex-[1] m-1 ">
        <form
          ...
          onSubmit={(e) => {
            e.preventDefault();
            roomid !== "" && socket.emit("join_room", roomid);
          }}
        >
          <input
            placeholder="enter room id"
            className="border-black border-2 ml-4"
            value={roomid}
        ...
        {/* game window */}
        <div
          className={`flex flex-col h-3/5 m-4 w-full border-4 border-black rounded-lg bg-[url(${imgadd})] bg-cover bg-no-repeat`}
        >
          <span className="border border-black text-black bg-white w-14 text-center m-2">
            Turn {turn}
          </span>
            ...
            moves.map((move) => {
              return (
                <div className="h-[95%] border-black border-2 w-1/4 rounded">
                  <button
                    class="bg-red-200 hover:bg-red-300 active:bg-red-400 text-black font-bold py-2 px-4 h-full w-full"
                    onClick={() => handleAttack(movestats[move])}
                  >
                    <h3>{move}</h3>
                    <h3>
                      {move.length > 0 &&
                        movestats[move] != undefined &&
                        movestats[move]["PP"] &&
                        movestats[move]["PP"]}
                    </h3>
                  </button>
